...
description: >-
  Generate a sanitized UniFi Network support bundle, help the user review it locally,
  and prepare it for a bug report without posting it
---

# Prepare a UniFi Network Support Bundle

Use `unifi_get_support_bundle` to collect the smallest useful, sanitized support bundle. The server
...
Call `unifi_get_support_bundle(probe="summary")` unless the user selected and confirmed another
supported probe. Never route this tool through `*_execute`, `*_batch`, a relay, or a
multi-location fan-out.

If the tool is missing, recommend updating the `network` plugin or package and
restarting the MCP client. If the server cannot start or register tools, use
`$unifi-network-setup` for setup help and fall back to the bug form's manual version,
