...
`UNIFI_PROTECT_API_KEY_FILE` and `UNIFI_PROTECT_API_KEY_COMMAND` behave the same way. Set exactly one spelling per level: `UNIFI_PROTECT_PASSWORD` next to `UNIFI_PROTECT_PASSWORD_FILE` refuses to start as ambiguous. A missing, unreadable, empty, multi-line or oversized file, an unresolvable or failing helper, and a helper that exceeds its 30-second timeout all refuse to start (exit code 6) with the variable name in the log; neither the file's contents nor anything the helper writes is ever logged.
